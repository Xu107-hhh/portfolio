...
export const CASE_STUDIES: CaseStudy[] = [
  {
    ...
    subTools: [
      // —— 课件加工（4）——
      { name: "PPT 静态加工工具", form: "GUI 工具 · 已推广", group: "课件加工", desc: "4 层语义 pipeline（输入理解→页面规划→生产执行→校验反馈），跨学段 + 跨学科 + 跨输入格式验证，单 case 40-50 秒" },
      { name: "理科静态加工 Web 版", form: "网页应用 · 已推广", group: "课件加工", desc: "3 阶段加工：基础排版 / 图片重绘抠图 / 思维导图转原生对象；思维导图等矢量元素结构对齐原稿并转为可编辑对象，产出供制作老师接手精修的初稿" },
      { name: "英语 PPT 提效主线（wenke）", form: "活跃主线 · 结构化生成", group: "课件加工", desc: "英语课件结构化生成唯一真源（7 月产品文档指定）：上游 HTML 数据革新 + 基础模板，v0.4.x 持续迭代，交付 v1.2 完整工程交接包；通用化模板与动画完善推进中" },
      { name: "HTML 交互课件", form: "网页课件 · POC 在线", group: "课件加工", desc: "以「剧本 JSON + 基础 HTML + 学科模板」生成可播放、可互动、可在线编辑的网页课件；小学英语 POC 已在线发布，63 个原始 SID 规范化为 45 个正确分支播放步骤" },
      // —— 动画制作（4）——
      { name: "动画注入插件（PPAM）", form: "PPT 插件 · 已推广", group: "动画制作", desc: "7 项动画功能，与静态工具文本框拆分协同；自动注入流程跑通 79 个动画动作，r10 播放基线验证通过" },
      { name: "自动动画注入 injection-lab", form: "实验项目", group: "动画制作", desc: "面向全自动动画注入的实验流水线（区别于人工 PPAM 基线），main@v0.3.0 已合并、feat@v0.4.0 运行适配中——从半自动到全自动的下一步探索" },
      { name: "数学动画自动渲染流水线", form: "自动化流水线 · 技术储备", group: "动画制作", desc: "「剧本 + PPT 版面 + 动画指令」→ 可渲染视频切片：翻译层、执行层、全局状态归一化、切片渲染、正确 / 错误分支分别渲染并拼接为互动题视频；程序兜底跨切片坐标跳变、字体状态、排版碰撞；接入 Manim 图形包支持点线边高亮，单条视频生成到渲染约 20-30 分钟；完成 Manim vs PPT（ppt-master）双路线技术选型判断——批量提效走 Manim、精修质感走 PPT" },
      { name: "AI 教育动画生产辅助平台", form: "Web 平台 · 收缩聚焦", group: "动画制作", desc: "以剧本项目为单位整合全流程：剧本解析（自动提取标题/角色/场景/切片/分支）→ 角色 / 场景参考图约束跨切片一致性 → 切片制作（首帧图 + 视频提示词 + 视频片段，支持首帧 / 尾帧 / 中间帧 / 参考图多种约束）→ 统一导出；多模型接入并对接公司统一网关。设定可验证的阶段目标口径：单剧本周期 5 天→3.5 天以内、新人上手 1 天→半天、图片可用率 ≥70%（Next.js）" },
      // —— 录屏剪辑（1）——
      { name: "录屏后处理工具 V2", form: "GUI + Skill · 已推广", group: "录屏剪辑", desc: "PPT 标记导出 → 视频切片 → S 编号重命名 → ZIP 打包四步串联，GUI 与 Claude Code Skill 双入口（主推 GUI）；把「负责人集中代跑」转变为「实习生自助处理、负责人只处理异常」，10 剧本从 1 人串行 5h 提升至 10 人并行 20min/人" },
      // —— 资料与素材配套（2）——
      { name: "剧本资料整理插件（资料导出）", form: "浏览器插件 · 试用后叫停", group: "资料与素材配套", desc: "教研资料导出与进度评审自动化，实测单剧本节省约 30 分钟（约 50 切片资料导出 + 进度评审表）；部分试用后因数据合规风险主动叫停，推动开发提供官方导出接口替代，沉淀「合规优先」判断复盘" },
      { name: "生图去水印扩展（调研引入）", form: "浏览器扩展 · 已进生产流程", group: "资料与素材配套", desc: "识别团队在豆包 / Dola / 千页提取无水印图片视频的共性需求后，调研引入现成扩展（无印豆包 · 素材提取）并推广使用——确定性需求优先用现成方案，不自研重复造轮子；调研引入，未计入 11 项工具卡" },
      // —— 能力底座（1）——
      { name: "AI Skills 工具箱", form: "AI Skill / CLI Harness · 能力底座", group: "能力底座", desc: "定制 Photoshop CLI harness 与 dashi-ppt 生成 Skill，横向研究 ppt-master / dashi-ppt / baoyu-design 等开源方案并二次开发，为静态加工工具提供能力底座" },
    ],
    reflection:
