...
export const CASE_STUDIES: CaseStudy[] = [
  {
    ...
    subTools: [
      ...
      { name: "听写判错数据分析", form: "数据分析", group: "学习机数据线", desc: "基于近 8 万条真实判错数据完成归因分析：抽样 200 条人工复核建模、归纳 16 类问题模式 → 候选规则全量扫描定位问题分布 → 80 条定向盲验；区分学生真实写错与系统误判，输出判题规则 / 答案配置 / 识别 SDK 的 P0-P2 分级优化建议" },
      { name: "手写 Badcase 打标工作台", form: "打标工作台", group: "学习机数据线", desc: "W/T/R 三元判断框架（固定 R↔T → T↔W 顺序）、盲看-揭示两阶段流程防标注偏见、AI 预填 + 人工核查、B1–B5 分批抽样与检查点准入；判断规则随 mentor 会议迭代至 V2.3.2，新问题类型须 ≥3 条独立样本且覆盖 2 个班级才正式建立" },
    ],
    reflection:
