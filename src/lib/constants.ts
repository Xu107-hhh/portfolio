...
export const CASE_STUDIES: CaseStudy[] = [
  {
    ...
    approach: [
      ...
      "识别图片素材去水印的共性需求后，调研引入现成浏览器扩展（无印豆包 · 素材提取）供团队在豆包 / Dola / 千页一键提取无水印图片与视频，进入真实生产流程——确定性需求优先用现成方案，不自研重复造轮子",
    ...
    subTools: [
      ...
      { name: "英语 PPT 提效主线（wenke）", form: "活跃主线 · 自动化流水线", desc: "英语课件结构化生成唯一真源（7 月产品文档指定）：上游 HTML 数据革新 + 基础模板，v0.4.x 持续迭代，交付 v1.2 完整工程交接包" },
      { name: "自动动画注入 injection-lab", form: "实验项目", desc: "面向全自动动画注入的实验流水线（区别于人工 PPAM 基线），main@v0.3.0 已合并、feat@v0.4.0 运行适配中——从半自动到全自动的下一步探索" },
