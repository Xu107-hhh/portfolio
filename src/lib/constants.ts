...
export const EXPERIENCE = [
  {
    ...
    details: [
      "产品矩阵规划：先实境体验动画生产流程约一周，再协同运营、教研、制作、剪辑完成全链路调研与问题定义，搭建覆盖课件加工、PPT+AI 动画制作、录屏剪辑 3 大主环节的提效产品矩阵，沉淀 11 项工具与作品、其中 4 款进入真实生产流程，并设定可验证目标口径（单剧本周期 5 天→3.5 天、素材可用率图 ≥70% / 视频 ≥50%）",
...
export const CASE_STUDIES: CaseStudy[] = [
  {
    ...
    subTools: [
      ...
      { name: "浏览器插件（资料导出）", form: "浏览器插件", desc: "教研资料导出与进度评审自动化，实测单剧本节省约 30 分钟（约 50 切片资料导出 + 进度评审表）；部分试用后因数据合规风险主动叫停，推动开发提供官方导出接口替代，沉淀「合规优先」判断复盘" },
    ...
    dataNote:
      "数据口径：剪辑单剧本由 30-60 分钟专人操作转为约 20-30 分钟无人值守；剧本资料整理由约 30 分钟压缩至 3~5 分钟；40% 首帧可用率为对应测试场景数据。不引用无测算记录的整体百分比，单点实测数据优先。",
  },
  {
    slug: "exam-designer",
    title: "智慧试卷设计师",
