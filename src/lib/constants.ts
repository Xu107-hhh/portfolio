...
//   "多款工具进真实生产" | "71 条素材自动命名" | "30 分钟→3~5 分钟资料整理"
//   "13+ 生成模型评测" | "89% 同事跑通" | "五级证据分级" | "Exit Criteria 阶段路线图"
...
export const ABOUT = {
  paragraphs: [
    "我是许隆鑫，天津师范大学情报学硕士研究生（数据科学方向），研究方向为人工智能应用。实践覆盖大模型评测、Prompt Engineering、RAG、AI Agent、多智能体系统和生产工具落地，擅长把模糊的「AI 提效」需求拆成场景、流程、验证指标与可交付方案。",
...
export const EXPERIENCE = [
  {
    ...
    summary: "阿里战投 · 准独角兽 · 50 人制作团队提效产品矩阵 + 学习机功能数据分析",
    details: [
      ...
      "剪辑自动化工具：将专人 30-60 分钟的连续脚本操作整合为「扫号、切段、贴标、打包」自动加工流程，封装 GUI 工具与 Claude Code Skill 双形态并内部推广——单剧本约 20 分钟无人值守，71 条素材零误差自动命名",
...
export const CASE_STUDIES: CaseStudy[] = [
  {
    ...
    tagline: "主要负责教学动画生产链 AI 提效 · 7 个月 11 项工具与作品",
    ...
    summary:
      "在精准学（阿里战投 · 准独角兽）主要负责教学动画生产链的 AI 提效产品矩阵，三条产品线并行——AI 动画生产辅助平台（流程整合）、数学 PPT / 动画自动化（结构化题型路线验证）、剪辑提效 V2（交付后处理）；7 个月内推动多款工具落地推广，覆盖数学 / 理科 / 文科三大学科线，并为每条线设定可验证的效率与质量目标口径。",
    problem:
      "教学动画生产依赖大量规则化重复劳动（PPT 静态复刻、录屏后处理、素材整理命名），工作高度集中在少数负责人手里串接多脚本，成为产能瓶颈；而一站式「剧本→成片」又受限于生成式 AI 的素材可用率，难以一步到位。",
    approach: [
    ...
    impact: [
      ...
      { value: "30→20 min", label: "单剧本剪辑处理" },
      { value: "30→3~5 min", label: "单剧本资料整理" },
      { value: "71 条", label: "素材零误差命名" },
    ],
    ...
    shots: [
      ...
      { src: "/images/gallery/real-annotation-workbench.png", alt: "手写 Badcase 打标工作台的单样本盲看复核界面", caption: "手写 Badcase 打标工作台——盲看-揭示两阶段复核（S121 样本）", w: 1440, h: 2786 },
      { src: "/images/gallery/real-browser-plugin.png", alt: "剧本资料汇总导出浏览器插件弹窗", caption: "剧本资料导出插件弹窗——自动采集切片数据并导出 HTML / XLSX", w: 400, h: 560 },
    ],
    subTools: [
      { name: "AI 教育动画生产辅助平台", form: "Web 平台", desc: "以剧本项目为单位整合全流程：剧本解析（自动提取标题/角色/场景/切片/分支）→ 角色 / 场景参考图约束跨切片一致性 → 切片制作（首帧图 + 视频提示词 + 视频片段，支持首帧 / 尾帧 / 中间帧 / 参考图多种约束）→ 统一导出；多模型接入并对接公司统一网关。设定可验证的阶段目标口径：单剧本周期 5 天→3.5 天以内、新人上手 1 天→半天、图片可用率 ≥70%（Next.js）" },
      { name: "数学动画自动渲染流水线", form: "自动化流水线", desc: "「剧本 + PPT 版面 + 动画指令」→ 可渲染视频切片：翻译层、执行层、全局状态归一化、切片渲染、正确 / 错误分支分别渲染并拼接为互动题视频；程序兜底跨切片坐标跳变、字体状态、排版碰撞；接入 Manim 图形包支持点线边高亮，单条视频生成到渲染约 20-30 分钟；完成 Manim vs PPT（ppt-master）双路线技术选型判断——批量提效走 Manim、精修质感走 PPT" },
      { name: "PPT 静态加工工具", form: "GUI 工具", desc: "4 层语义 pipeline（输入理解→页面规划→生产执行→校验反馈），跨学段 + 跨学科 + 跨输入格式验证，单 case 40-50 秒" },
      { name: "理科静态加工 Web 版", form: "网页应用", desc: "3 阶段加工：基础排版 / 图片重绘抠图 / 思维导图转原生对象；思维导图等矢量元素结构对齐原稿并转为可编辑对象，产出供制作老师接手精修的初稿" },
      { name: "动画注入插件（PPAM）", form: "PPT 插件", desc: "7 项动画功能，与静态工具文本框拆分协同；自动注入流程跑通 79 个动画动作，r10 播放基线验证通过" },
      { name: "浏览器插件（资料导出）", form: "浏览器插件", desc: "教研资料导出与进度评审自动化，实测单剧本节省约 30 分钟（约 50 切片资料导出 + 进度评审表）；后因涉及数据合规主动叫停，沉淀「合规优先」判断复盘" },
      { name: "AI Skills 工具箱", form: "AI Skill / CLI Harness", desc: "定制 Photoshop CLI harness 与 dashi-ppt 生成 Skill，横向研究 ppt-master / dashi-ppt / baoyu-design 等开源方案并二次开发，为静态加工工具提供能力底座" },
      { name: "录屏后处理工具 V2", form: "GUI + Claude Code Skill", desc: "PPT 标记导出 → 视频切片 → S 编号重命名 → ZIP 打包四步串联，GUI 与 Claude Code Skill 双入口（主推 GUI）；把「负责人集中代跑」转变为「实习生自助处理、负责人只处理异常」，10 剧本从 1 人串行 5h 提升至 10 人并行 20min/人" },
      { name: "HTML 交互课件", form: "网页课件", desc: "以「剧本 JSON + 基础 HTML + 学科模板」生成可播放、可互动、可在线编辑的网页课件；小学英语 POC 已在线发布，63 个原始 SID 规范化为 45 个正确分支播放步骤" },
      { name: "听写判错数据分析", form: "数据分析", desc: "基于近 8 万条真实判错数据完成归因分析：抽样 200 条人工复核建模、归纳 16 类问题模式 → 候选规则全量扫描定位问题分布 → 80 条定向盲验；区分学生真实写错与系统误判，输出判题规则 / 答案配置 / 识别 SDK 的 P0-P2 分级优化建议" },
      { name: "手写 Badcase 打标工作台", form: "打标工作台", desc: "W/T/R 三元判断框架（固定 R↔T → T↔W 顺序）、盲看-揭示两阶段流程防标注偏见、AI 预填 + 人工核查、B1–B5 分批抽样与检查点准入；判断规则随 mentor 会议迭代至 V2.3.2，新问题类型须 ≥3 条独立样本且覆盖 2 个班级才正式建立" },
    ],
    reflection:
  ...
  },
  {
    ...
    approach: [
      ...
      "混合爬虫两阶段调度：aiohttp 异步爬取静态页面优先，JS 重度页面自动切换 Chromium 无头浏览器补救，信息源从早期 8 类扩展至 136 个全球权威源",
  ...
  },
  {
    ...
    reflection:
      "评测的价值在「可复现 + 可解释」：固定基准、固定提示词、固定输入条件后，模型间差异才能归因于能力本身而非随机性。结论也不能停在「谁更强」，要落到「哪种组合在什么场景下用」——文献罗盘 + 上传文献是构建专业知识体系的最佳组合。",
    dataNote:
  ...
  },
  {
    ...
    problem:
      "小微门店没有 IT 运维，官网与预约登记若依赖云端服务，就有成本、部署与数据归属三重顾虑——需要一套店员电脑双击即用、数据完全留在本地的轻量方案；且正式营业前的商品与门店信息必然是占位状态，上线替换路径必须从第一天就设计好。",
    approach: [
    ...
    reflection:
      "AI 编程 Agent 把「一个人做出能运行的系统」变成现实之后，产品经理的增量价值进一步转移到需求边界、上线清单与验收标准上——这些恰好是 AI 最不擅长的部分。",
    dataNote:
...
export const METHODOLOGY = {
  title: "方法论沉淀",
  description:
    "在科研情报项目与 7 个月产品实习中沉淀的验证纪律：用对照实验约束评测，用阶段准出条件约束节奏，用证据分级约束表达，让每一步推进都有可复现的口径。",
  blocks: [
    {
      index: "01",
